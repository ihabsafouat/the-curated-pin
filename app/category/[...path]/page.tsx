...
export default async function CategoryPage({ params }: { params: Promise<{ path: string[] }> }) {
  ...
  return <main><SubHeader/><Breadcrumbs items={breadcrumbs}/><StructuredData data={categorySchema}/>
    ...
    <section className="listing shell"><div className="listingHead"><h2>Latest in {category.name}</h2><span>{items.length} curated guide{items.length === 1 ? "" : "s"}</span></div>{items.length ? <div className="listingGrid">{items.map((article) => <article key={article.slug}><a href={`/article/${article.slug}`}><MediaImage src={article.image} alt={article.imageAlt || `Preview for ${article.title}`} variant="card" loading="lazy" decoding="async" sizes="(max-width: 800px) 100vw, 380px"/></a><small>{article.category} · {article.readTime}</small><h3><a href={`/article/${article.slug}`}>{article.title}</a></h3><p>{article.dek}</p><a className="readMore" href={`/article/${article.slug}`}>Read guide →</a></article>)}</div> : <div className="categoryEmpty"><small>EDITORIAL QUEUE</small><h3>Guides for this section are being curated.</h3><p>This category stays followable but is marked noindex until it contains published editorial content. That keeps thin taxonomy pages out of search while preserving the internal structure.</p></div>}</section>
    <SubFooter/>
  </main>;
}
